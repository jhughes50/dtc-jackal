...
class SkinExtractionHSV:
    ...
    def extract_skin(self, image, ldmks):
        ...
        # darker skin
        lower_threshold = np.array([1, 48, 20], dtype=np.uint8)
        upper_threshold = np.array([20, 255, 255], dtype=np.uint8)

        # lower_threshold = np.array([0, 15, 30], dtype=np.uint8)
        ...
        # remove all black pixels from the image and get cropped_skin_im
        cropped_skin_im = skin_image

        #Find the bounding box of the non-black regions in the mask
        x, y, w, h = cv2.boundingRect(skin_mask)

        # Crop the skin image using the bounding box
        cropped_skin_im = skin_image[y:y+h, x:x+w]
