...
class SignalProcessing():
    ...
    # change from videoFileName to frames
    def extract_raw(self, frames):
        """
    ...
    # change from VideoFileName to frames
    def extract_raw_holistic(self, frames):
        """
            ...
            for frame in frames:
               # convert the BGR image to RGB.
                ...
                if results.multi_face_landmarks:
                    ...
                    # use hsv skin extraction if no face is detected
                    face_detected = False
                    # print("Face not detected, using hsv skin extraction")
                    cropped_skin_im, full_skin_im = skin_ex.extract_skin(image, ldmks)
    ...
    # change from videoFileName to frames
    def extract_holistic(self, frames):
        """
        ...
        with mp_face_mesh.FaceMesh(
            ...
            for frame in frames:
                ...
                if results.multi_face_landmarks:
                    ...
                    # dynamic skin regions
                    magic_ldmks = self.find_skin_regions(full_skin_im, region_type)
                    magic_ldmks = np.array(magic_ldmks, dtype=np.float32)
                    # print(magic_ldmks)#100x5
