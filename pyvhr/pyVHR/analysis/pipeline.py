...
class Pipeline():
    ...
    def run_on_video_multimethods(self, frames, 
                    winsize, 
                    ldmks_list=None,
                    cuda=True, 
                    roi_method='convexhull', 
                    roi_approach='holistic', 
                    methods=['cpu_CHROM, cupy_CHROM, cpu_LGI'], 
                    estimate='holistic', 
        ...
        if roi_approach == 'holistic':
            ...
            sig = sig_processing.extract_holistic(frames)
        elif roi_approach == 'patches':
            # SIG extraction with patches
            sig = sig_processing.extract_patches(frames, 'squares', 'mean')
        if verb:
    ...
    def run_on_video(self, frames, 
        ...
        ## add a new ROI method here, and get new threshold for hsv.
...
class DeepPipeline(Pipeline):
    ...
    def run_on_video(self, videoFileName, cuda=True, method='MTTS_CAN', bpm_type='welch', post_filt=False, verb=True, crop_face=False):
        ...
        if verb:
            print('\nProcessing Video: ' + videoFileName)
        fps = 30#get_fps(videoFileName)
        wsize = 6
        
        sp = SignalProcessing()
        frames = videoFileName #sp.extract_raw(videoFileName)
