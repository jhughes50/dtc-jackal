...
def filter_boxes(boxes, mode='size', keep=1):
    if mode == 'size':
        centers, scales = boxes_2_cs(boxes)
        idx = np.argsort(scales)[::-1].copy()
    elif mode == 'conf':
        confidence = boxes[:, 4]
        idx = torch.argsort(confidence).numpy()[::-1].copy()
...
def crop_from_box(img, box, rescale=1.2, res=(256,256), val=255):
    center, scale = box_2_cs(box)
    scale = scale * rescale
    crop = crop_img(img, center, scale, res=res, val=val)
    return crop


def crop_img(img, center, scale, res=(256,256), val=255):
    """Crop image according to the supplied bounding box."""
    # Upper left point
    ul = np.array(transform([1, 1], center, scale, res, invert=1))-1
    # Bottom right point
    ...
    new_img[new_y[0]:new_y[1], new_x[0]:new_x[1]] = img[old_y[0]:old_y[1], 
                                                        old_x[0]:old_x[1]]
    new_img = resize(new_img, res).astype(int)
    return new_img
